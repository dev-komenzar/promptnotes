...
- **I-N9**: `body_hash` は `body` から決定論的に導出され、`body` 変更時に必ず再計算される。外部プログラムによる `.md` ファイル変更の競合検出に使用される。本 slice は write 側（Flush）であり、競合検出（`is_stale` チェック）は application service 層の責務。Flush 発火時点ではユーザが編集中のため競合解決は不要（ユーザの編集が正）
