...
- **Note**（中核）
  ...
  - **削除 Undo 決定**（2026-06-16, Q5）:
    - Undo 有効期間 = トースト表示中のみ（仮 5 秒、UI Phase で確定）
    - トースト消失（時間切れ / 明示クローズ / 新規削除による置換）と同時に `RestoreDeletedNote` 不可
    - 復元手段はトースト内「元に戻す」ボタンのみ。`Cmd+Z` は採用しない（編集中 Undo と衝突回避）
    - トースト消失後は OS のゴミ箱からの手動復元に委ねる（アプリ責務外）
    - Undo stack を domain に持たない設計（in-memory に直近 1 件の path のみ保持）
