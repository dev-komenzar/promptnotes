...
- **Query** — 検索バーの文字列。case-insensitive substring + NFKC (compatibility normalization) 正規化で `body` と `tags` にマッチ
...
- **CQRS 的分割の根拠**：Note Capture は file IO / debounce / focus イベント / OS ゴミ箱 API の世界、
  Note Feed は in-memory index / NFKC (compatibility normalization) 正規化 / 検索文字列マッチの世界。
