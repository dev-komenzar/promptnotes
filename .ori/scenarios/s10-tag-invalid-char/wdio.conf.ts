...
export const config: WebdriverIO.Config = {
  ...
  onPrepare: async () => {
    tmpDir = mkdtempSync(tmpdir() + '/promptnotes-scenario-s10-');
    // S10 Given: Note A (body="hello", tags=[], updatedAt=t0)
    seedNote('20260620120000', 'hello');
    process.env.TAURI_TEST_STORAGE_DIR = tmpDir;
    console.log(`[scenario:s10] temp storage dir: ${tmpDir}`);
  },

  onComplete: async () => {
    if (tmpDir) {
      rmSync(tmpDir, { recursive: true, force: true });
      console.log(`[scenario:s10] cleaned temp dir: ${tmpDir}`);
    }
  },
};
