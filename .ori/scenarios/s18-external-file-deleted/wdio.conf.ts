...
export const config: WebdriverIO.Config = {
  ...
  onPrepare: async () => {
    tmpDir = mkdtempSync(tmpdir() + '/promptnotes-scenario-s18-');
    ...
    // S18 Given: storage_dir には Note A / Note B。外部削除はテスト本体が行う。
    seedNote(notesDir, '20260620120000', 'hello');
    seedNote(notesDir, '20260621090000', 'world');

    process.env.XDG_CONFIG_HOME = configHome;
    process.env.XDG_DATA_HOME = dataHome;
    process.env.TAURI_TEST_STORAGE_DIR = notesDir;
    console.log(`[scenario:s18] notes=${notesDir}`);
  },

  onComplete: async () => {
    if (tmpDir) {
      rmSync(tmpDir, { recursive: true, force: true });
      console.log(`[scenario:s18] cleaned temp dir: ${tmpDir}`);
    }
  },
};
