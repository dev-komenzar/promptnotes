...
function parseValidationSections(md) {
  const sections = [];
  const re = /^## Scenario[^\n{]*\{#([a-z][a-z0-9-]*)\}[ \t]*$/gm;
  let m;
  while ((m = re.exec(md)) !== null) {
    const id2 = m[1];
    if (sections.some((s) => s.id === id2)) continue;
    const title = m[0].replace(/^## Scenario[:\s]*/, "").replace(/\{#[a-z0-9-]+\}[ \t]*$/, "").trim();
    sections.push({ id: id2, title });
  }
  return sections;
}
async function listScaffoldedScenarios(cwd2) {
  const scenariosDir = join(cwd2, ".ori", "scenarios");
...
async function runListValidation(cwd2) {
  const validationPath = join(cwd2, ".ori", "domain", "validation.md");
  ...
  const scaffolded = new Set(await listScaffoldedScenarios(cwd2));
  ...
  const orphans = [...scaffolded].filter((id2) => !anchorIds.has(id2)).sort();
  if (orphans.length > 0) {
    console.error(
...
var listValidation = args.includes("--list-validation");
var id = args.find((a) => !a.startsWith("--")) ?? "";
var cwd = process.cwd();
if (listValidation) {
