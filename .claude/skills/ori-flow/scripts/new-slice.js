...
var typeArg = (() => {
  const idx = args.findIndex((a3) => a3 === "--type" || a3.startsWith("--type="));
  if (idx === -1) return "command";
  const a2 = args[idx];
  return a2.includes("=") ? a2.split("=")[1] : args[idx + 1] ?? "command";
})();
...
var tpl = await loadTemplate("slice-manifest.yaml.tpl");
var manifestContent;
if (tpl) {
...
> This file is a derived document. Edit the source manifest + domain docs and re-run \`/ori-flow run-slice ${id} phase=derive\`. Use \`/ori-sync\` if you need to edit here directly; ori will create a proposal for the upstream review.
...
var status = {
  ...
  beads: { epic: formatEpicId("slice", id), current_phase: null, completion: [] },
  phases: {},
  dirty: []
};
