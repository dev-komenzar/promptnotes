...
var require_stringify = __commonJS({
  "node_modules/.pnpm/yaml@2.9.0/node_modules/yaml/dist/stringify/stringify.js"(exports2) {
    ...
    function stringify(item, ctx, onComment, onChompKeep) {
      ...
      let tagObj = void 0;
      const node2 = identity.isNode(item) ? item : ctx.doc.createNode(item, { onTagObj: (o3) => tagObj = o3 });
  ...
  "node_modules/.pnpm/yaml@2.9.0/node_modules/yaml/dist/compose/resolve-block-scalar.js"(exports2) {
    ...
    function resolveBlockScalar(ctx, scalar, onError) {
      ...
      for (let i2 = 0; i2 < chompStart; ++i2) {
        const [indent, content3] = lines[i2];
        if (content3 === "" || content3 === "\r") {
...
var require_public_api = __commonJS({
  "node_modules/.pnpm/yaml@2.9.0/node_modules/yaml/dist/public-api.js"(exports2) {
    ...
    function parse3(src2, reviver, options2) {
      let _reviver = void 0;
...
function parseArchitectureSpec(raw) {
  const { data, content: content3 } = parseFrontmatter(raw);
...
function parseAgentSection(body) {
  const merged = {};
  for (const match of body.matchAll(/```yaml\s*\n([\s\S]*?)```/g)) {
    const doc = import_yaml.default.parse(match[1]);
    for (const [k2, v2] of Object.entries(doc)) merged[k2] = v2;
  }
  return merged;
}
function extractStructuredSections(raw) {
  const { content: content3 } = parseFrontmatter(raw);
  const sections = extractSections(content3);
  const out = { invariants: null, guardrails: null };
  for (const section of sections.ordered) {
    if (section.depth !== 2) continue;
    if (section.heading === "invariants") {
      const parsed = parseAgentSection(section.body);
      out.invariants = parsed.invariants ?? null;
    } else if (section.heading === "guardrails") {
      const parsed = parseAgentSection(section.body);
      out.guardrails = parsed.guardrails ?? null;
    }
  }
  return out;
}
function push2(issues2, file, guardrailId, message) {
  issues2.push({
    file,
    line: 1,
...
function checkLayerSets(spec, inv, issues2, file) {
...
function checkBoundaries(spec, raw, inv, issues2, file) {
...
function checkPublicEntry(spec, issues2, file) {
  for (const root of spec.roots) {
...
function checkCrossRoot(spec, issues2, file) {
...
function checkDecisionPoints(spec, raw, issues2, file) {
  const { data } = parseFrontmatter(raw);
  ...
  if (decisions) {
    ...
    const missing = required.filter((k2) => !(k2 in decisions));
...
async function runGuardrailsCheck(archPath) {
  ...
  try {
    ...
    push2(issues2, archPath, "g-1", `architecture.md \u304C ArchitectureSpec \u3068\u3057\u3066 parse \u3067\u304D\u306A\u3044: ${detail}`);
    return issues2;
  }
  const validators = /* @__PURE__ */ new Map([
    ...
    ["g-6", () => checkPublicEntry(spec, issues2, archPath)],
