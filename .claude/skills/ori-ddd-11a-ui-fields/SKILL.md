...
- 出力（**ファイル分割必須**）：
  ...
  - `.ori/domain/ui-fields/screen-<N>.md` — 各画面の入力項目（1 画面 1 ファイル = 1 node）
  - **全ファイル** に H2/H3 `{#id}` と `ori:` frontmatter（design.md §5）を持つ
    - `index.md`: `node_id: ui-field:index`, `type: ui-field`, `depends_on: [type-definitions:index, workflow:index]`
    - `screen-<N>.md`: `node_id: ui-field:screen-<N>`, `type: ui-field`, `depends_on: [workflow:<id>, type-definitions:index]`
...
- distill-ddd 上流との差分：画面分割、Phase 11a/11b 分離、`ori:` frontmatter（design.md §5）
