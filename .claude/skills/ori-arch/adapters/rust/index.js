...
// packages/arch-adapters/rust/src/index.ts
...
function sliceBase2(root) {
  const trimmed = root.slice_root === "." ? ensureTrailingSlash2(root.path) : ensureTrailingSlash2(`${root.path}/${root.slice_root}`);
  return trimmed;
}
function ensureTrailingSlash2(p) {
  return p.endsWith("/") ? p : `${p}/`;
}
function crateRoot(root) {
...
function applyPlaceholders(template, subs) {
  let out = template;
...
var adapter = {
  ...
  async export(spec, root, opts = {}) {
    ...
    let content = template.replaceAll('"__ORI_ROOT_ID__"', `"${escapeRustStr(root.id)}"`).replaceAll('"__ORI_ROOT_PATH__"', `"${escapeRustStr(root.path)}"`).replaceAll('"__ORI_SLICE_BASE__"', `"${escapeRustStr(base)}"`).replaceAll('"__ORI_PUBLIC_ENTRY__"', `"${escapeRustStr(root.public_entry)}"`);
