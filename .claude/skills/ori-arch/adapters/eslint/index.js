...
// packages/arch-adapters/eslint/src/index.ts
...
// packages/arch-adapters/eslint/src/index.ts
var PATTERN_EXT = "{ts,tsx,js,jsx,mts,cts,mjs,cjs}";
function defaultTemplatesDir() {
  return join(dirname(fileURLToPath(import.meta.url)), "templates");
}
function appPrefix(root) {
...
function buildDependencyRules(spec, root, rules) {
  const set = spec.layer_sets[root.layer_set];
...
function buildForbiddenBlocks(spec, root, matchers, prefix) {
  const set = spec.layer_sets[root.layer_set];
  if (!set) return [];
  const blocks = [];
  for (const fi of set.rules.forbidden_imports ?? []) {
    ...
    if (!glob) continue;
    const paths = fi.modules.map((module2) => ({
      name: module2,
      message: fi.reason ?? `forbidden by .ori/architecture.md (from=${fi.from})`
    }));
    blocks.push({
      files: [glob],
      rules: {
