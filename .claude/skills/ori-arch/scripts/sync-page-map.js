...
// packages/skills/ori-arch/src/sync-page-map.ts
...
function replaceClose(index, string, close, replace, head = string.slice(0, Math.max(0, index)) + replace, tail = string.slice(Math.max(0, index + close.length)), next2 = tail.indexOf(close)) {
  return head + (next2 < 0 ? tail : replaceClose(next2, tail, close, replace));
}
...
// packages/skills/ori-arch/src/sync-page-map.ts
var DEFAULT_PAGE_MAP_MARKER = "phase-11b";
var SCREEN_PREFIX = "screen-";
var SCREEN_SUFFIX = ".md";
var ARCHITECTURE_PATH = ".ori/architecture.md";
var UI_FIELDS_DIR = ".ori/domain/ui-fields";
var SECTION_HEADER = "## Page Map";
var GROUP_KINDS = ["ui-widget", "ui-page"];
...
function isGroupKind(s2) {
  return GROUP_KINDS.includes(s2);
}
function parseScreen(file, raw) {
  ...
  const groups = [];
  for (const r3 of extractRefs(coherence.depended_by)) {
    if (isGroupKind(r3.key)) groups.push({ kind: r3.key, id: r3.value });
  }
  const dependsOn = [];
  for (const r3 of extractRefs(coherence.depends_on)) {
    if (r3.key === "screen") continue;
    dependsOn.push({ kind: r3.key, id: r3.value });
  }
  return { screenId, file, groups, dependsOn };
}
function compareScreenIds(a2, b2) {
  const an = Number(a2.slice(SCREEN_PREFIX.length));
  const bn = Number(b2.slice(SCREEN_PREFIX.length));
  if (Number.isFinite(an) && Number.isFinite(bn)) return an - bn;
  return a2.localeCompare(b2);
}
var KIND_ORDER = ["ui-widget", "ui-page"];
function collectPageMap(screens2) {
  const byGroup = /* @__PURE__ */ new Map();
  for (const s2 of screens2) {
    for (const g3 of s2.groups) {
      const key = `${g3.kind}:${g3.id}`;
      let agg = byGroup.get(key);
      if (!agg) {
        agg = { kind: g3.kind, id: g3.id, screens: /* @__PURE__ */ new Set(), sources: /* @__PURE__ */ new Set(), dependsOn: /* @__PURE__ */ new Set() };
        byGroup.set(key, agg);
      }
      agg.screens.add(s2.screenId);
      agg.sources.add(s2.file);
      for (const d2 of s2.dependsOn) agg.dependsOn.add(d2.id);
    }
  }
  const entries2 = [];
  for (const v2 of byGroup.values()) {
    entries2.push({ kind: v2.kind, id: v2.id, dependsOn: [...v2.dependsOn].sort(), screens: [...v2.screens].sort(compareScreenIds), sources: [...v2.sources].sort() });
  }
  entries2.sort((a2, b2) => {
    const ka = KIND_ORDER.indexOf(a2.kind);
    const kb = KIND_ORDER.indexOf(b2.kind);
    if (ka !== kb) return ka - kb;
    return a2.id.localeCompare(b2.id);
  });
  return entries2;
}
function renderPageMapBody(entries2) {
...
function buildDelimiters(marker2) {
  return {
    begin: `<!-- BEGIN ori-distill ${marker2} auto-generated; do not edit between markers -->`,
    end: `<!-- END ori-distill ${marker2} auto-generated -->`
  };
}
function applyPageMapSection(archMd, body2, marker2) {
  const { begin, end } = buildDelimiters(marker2);
  const block = `${begin}
${body2.trimEnd()}
${end}`;
...
var cwd = process.cwd();
var specPath = flag("spec") ? join(cwd, flag("spec")) : join(cwd, ARCHITECTURE_PATH);
var uiFieldsDir = flag("ui-fields-dir") ? join(cwd, flag("ui-fields-dir")) : join(cwd, UI_FIELDS_DIR);
var markerArg = flag("marker");
...
var marker = await readMarker();
var fileNames = await (async () => {
  const entries2 = await readdir(uiFieldsDir, { withFileTypes: true });
  return entries2.filter((e2) => e2.isFile() && e2.name.startsWith(SCREEN_PREFIX) && e2.name.endsWith(SCREEN_SUFFIX)).map((e2) => e2.name).sort(compareScreenIds);
})();
var screens = [];
...
var entries = collectPageMap(screens);
var body = renderPageMapBody(entries);
var current = await readFile(specPath, "utf8");
var next = applyPageMapSection(current, body, marker);
