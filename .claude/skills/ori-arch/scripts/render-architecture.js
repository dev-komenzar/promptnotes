...
var require_public_api = __commonJS({
  "node_modules/.pnpm/yaml@2.9.0/node_modules/yaml/dist/public-api.js"(exports2) {
    ...
    function parse2(src2, reviver, options2) {
      let _reviver = void 0;
...
// packages/skills/ori-arch/src/render-architecture.ts
...
// packages/skills/ori-arch/src/render-architecture.ts
var DEFAULT_BC = "task-management";
function usage() {
...
function parseArgs(argv2) {
  ...
  for (let i2 = 0; i2 < argv2.length; i2++) {
    const a2 = argv2[i2];
    const eq = a2.indexOf("=");
    const key = eq === -1 ? a2 : a2.slice(0, eq);
    const valInline = eq === -1 ? void 0 : a2.slice(eq + 1);
    const take = () => {
      if (valInline !== void 0) return valInline;
      const next = argv2[++i2];
      if (next === void 0) {
...
` + usage());
    ...
    switch (key) {
      ...
      case "--help":
        ...
        break;
      default:
        process.stderr.write(`Unknown argument: ${a2}
...
async function resolveTemplate(patternsDir, pattern, stack) {
  ...
  if (!await exists(patternDir)) {
    const available2 = await listDirs(patternsDir);
    process.stderr.write(
      `Unknown pattern: "${pattern}"
Available patterns: ${available2.length ? available2.join(", ") : "(none found)"}
`
    );
    process.exit(2);
  }
...
async function resolveAppName(dest, override) {
...
`
  ...
  let parsed;
  try {
    parsed = (0, import_yaml.parse)(await readFile(configPath, "utf8"));
  } catch (err) {
    process.stderr.write(
...
`
  ...
  const apps = parsed?.ori?.workspace?.apps;
...
function render(tpl, vars) {
  let unresolved = null;
  const out = tpl.replace(/\{\{(\w+)\}\}/g, (whole, key) => {
    const v2 = vars[key];
    if (v2 === void 0) {
      unresolved ??= whole;
      return whole;
